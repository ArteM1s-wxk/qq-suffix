from qq_suffix.config import DEFAULT_HOTKEY, DEFAULT_NEWLINE, DEFAULT_SUFFIX, load_config, save_config
...
def test_load_config_returns_default_on_empty_suffix(tmp_path):
    path = tmp_path / "config.json"
    save_config(path, "", True, "ctrl+enter")
    config = load_config(path)
    assert config.suffix == DEFAULT_SUFFIX
    assert config.newline is True
    assert config.hotkey == "ctrl+enter"
...
def test_load_config_falls_back_hotkey_on_empty(tmp_path):
    path = tmp_path / "config.json"
    path.write_text('{"suffix": "x", "hotkey": ""}', encoding="utf-8")
    config = load_config(path)
    assert config.hotkey == DEFAULT_HOTKEY
